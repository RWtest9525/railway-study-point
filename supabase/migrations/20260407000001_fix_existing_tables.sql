-- =====================================================
-- FIX EXISTING TABLES - Add missing category_id columns
-- This migration adds category_id to existing tables
-- Created: 2026-04-07
-- =====================================================
...
-- Add category_id to questions table if it doesn't exist
DO $$ 
BEGIN 
  IF NOT EXISTS (
    ...
    ALTER TABLE questions ADD COLUMN category_id UUID REFERENCES categories(id) ON DELETE SET NULL;
    RAISE NOTICE 'Added category_id column to questions table';
  ELSE
    RAISE NOTICE 'category_id column already exists in questions table';
  END IF;
END $$;

-- Add category_id to exams table if it doesn't exist
DO $$ 
BEGIN 
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
