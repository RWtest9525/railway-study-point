    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Monitor and manage user premium subscriptions</p>
        </div>
        <button
          onClick={loadUsers}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-white">{stats.never}</p>
              <p className="text-xs text-gray-400">Never Subscribed</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-700/50">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Subscription Status</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest hidden md:table-cell">Valid Until</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest hidden lg:table-cell">Joined At</th>
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-gray-700/30 transition group">
                        ...
                        <div className="flex items-center gap-2">
                          {isActive ? (
                            <>
                              ...
                              <span className="text-green-400 font-medium text-sm">Active Premium</span>
                            </>
                          ) : isExpired ? (
                            <>
                              <XCircle className="w-4 h-4 text-orange-500" />
                              <span className="text-orange-400 font-medium text-sm">Expired</span>
                            </>
                          ) : (
                            <>
                              <AlertCircle className="w-4 h-4 text-gray-500" />
                              <span className="text-gray-500 font-medium text-sm">Never Subscribed</span>
                            </>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 hidden md:table-cell">
                        {u.premium_until ? (
                          <div className="flex flex-col">
                            <span className="text-white text-sm">{new Date(u.premium_until).toLocaleDateString()}</span>
                            <span className="text-gray-500 text-xs">{new Date(u.premium_until).toLocaleTimeString()}</span>
                          </div>
                        ) : (
                          <span className="text-gray-600 text-sm italic">No subscription</span>
                        )}
                      </td>
