...
export function SubjectQuizzes() {
  ...
  const loadExams = async () => {
    try {
      ...
      console.error('Error loading subject quizzes:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
        {loading ? (
          ...
          <div className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-2xl border p-8 text-center`}>
            <p className={`${isDark ? 'text-gray-400' : 'text-gray-500'}`}>No subject quizzes available yet.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {exams.map((exam) => (
              <div
                key={exam.id}
                className={`${isDark ? 'bg-gray-800 border-gray-700 hover:border-green-500' : 'bg-white border-gray-200 hover:border-green-400'} rounded-2xl border p-5 transition cursor-pointer`}
